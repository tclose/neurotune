...
cell_9ml = os.path.join('/home', 'tclose', 'git', 'kbrain', '9ml', 'neurons',
                      'Golgi_Solinas08.9ml')
...
plain_objective = PhasePlaneHistObjective(reference_trace, resample=None,
                                          kernel_stdev=None)
resamp_objective = PhasePlaneHistObjective(reference_trace, kernel_stdev=None)
conv_objective = PhasePlaneHistObjective(reference_trace)

objectives = [resamp_objective, conv_objective, plain_objective]

resamp_objective.plot_d_dvdt(reference_trace,
                             show='/home/tclose/Documents/v_dvdt.pkl')

for obj, title in zip(objectives, ('resampled', 'convolved', 'plain')):
    obj.plot_hist(reference_trace, show=os.path.join('/home', 'tclose',
                                                     'Documents',
                                                     title + '.pkl'))
    plt.title(title)
